...
export default function Actions() {
  ...
  const readerAddress = getContract(chainId, "PhamousUiDataProvider");
  ...
  const pnlUrl = `${getServerBaseUrl(
    chainId
  )}/pnl?account=${checkSummedAccount}`;
  ...
  const [orders, updateOrders] = useAccountOrders(
    flagOrdersEnabled,
    checkSummedAccount
  );
  ...
  return (
    <div className="Actions">
      {checkSummedAccount.length > 0 && (
        <div className="Actions-section">Account: {checkSummedAccount}</div>
      )}
      {shouldShowPnl && (
