...
export default function OrdersOverview() {
  ...
  const { infoTokens } = Api.useInfoTokens(
    library,
    chainId,
    active,
    undefined,
  ...
  if (stats) {
    ...
    executedTotal =
      stats.executedDecrease + stats.executedIncrease + stats.executedSwap;
    cancelledTotal =
      stats.cancelledDecrease + stats.cancelledIncrease + stats.cancelledSwap;
  }
    ...
    <div className="Orders-overview">
      {stats && (
        <p className="Orders-overview-stats">
          Total active: {openTotal}, executed: {executedTotal}, cancelled:{" "}
          {cancelledTotal}
          <br />
          Increase active: {stats.openIncrease}, executed:{" "}
          {stats.executedIncrease}, cancelled: {stats.cancelledIncrease}
          <br />
          Decrease active: {stats.openDecrease}, executed:{" "}
          {stats.executedDecrease}, cancelled: {stats.cancelledDecrease}
          <br />
          Swap active: {stats.openSwap}, executed: {stats.executedSwap},
          ...
          {orders.map((order) => {
            ...
            if (type === SWAP) {
              ...
              if (fromToken && toToken) {
                ...
                prefix =
                  (order.triggerAboveThreshold && !invert) ||
                  (!order.triggerAboveThreshold && invert)
                    ? "> "
                    : "< ";
                shouldExecute =
                  markExchangeRate && markExchangeRate.lt(order.triggerRatio);
                nearExecute =
                  markExchangeRate &&
                  markExchangeRate.lt(
                    order.triggerRatio.mul(100).div(NEAR_TRESHOLD)
                  );
              ...
              const purchaseToken = getTokenInfo(
                infoTokens,
                order.purchaseToken
              );
              ...
              if (
                indexToken &&
                collateralToken &&
                (order.type === DECREASE || purchaseToken)
              ) {
                markPrice = order.triggerAboveThreshold
                  ? indexToken.minPrice
                  : indexToken.maxPrice;
              } else {
              ...
              if (markPrice) {
                ...
                nearExecute = order.triggerAboveThreshold
                  ...
                  : markPrice.lt(
                      order.triggerPrice.mul(100).div(NEAR_TRESHOLD)
                    );
