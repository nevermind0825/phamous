...
export default function OrdersList(props) {
  ...
  const renderHead = useCallback(() => {
    const isAllOrdersSelected =
      ...
      cancelOrderIdList?.length === orders.length;
    return (
  ...
  const renderLargeList = useCallback(() => {
    ...
    return orders.map((order) => {
      ...
      const maximisePrice =
        (order.type === INCREASE && order.isLong) ||
      ...
      if (order.type === DECREASE) {
        const positionForOrder = getPositionForOrder(
          account,
          order,
          positionsMap
        );
          ...
          <td>
            <Tooltip
              ...
              renderContent={() => {
                return (
                  <>
                    The price that the order can be executed at may differ
                    slightly from the chart price as market orders can change
                    the price while limit / trigger orders cannot.
                  </>
                );
              }}
            />
                ...
                <Tooltip
                  ...
                  renderContent={() => {
                    return (
                      <>
                        The price that the order can be executed at may differ
                        slightly from the chart price as market orders can
                        change the price while limit / trigger orders cannot.
                      </>
                    );
                  }}
                />
