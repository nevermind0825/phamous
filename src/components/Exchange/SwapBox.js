...
export default function SwapBox(props) {
  ...
  const [shortCollateralAddress, setShortCollateralAddress] =
    useLocalStorageByChainId(
      chainId,
  ...
  //     return "https://www.phamous.house/arbitrum/leaderboard";
  //   }
  //   return "https://www.phamous.house";
  // };
  ...
  const needPositionRouterApproval =
    (isLong || isShort) && isMarketOrder && !positionRouterApproved;
  const prevNeedPositionRouterApproval = usePrevious(
  ...
  const { data: minExecutionFee } = useSWR(
  ...
  const maxToTokenOut = useMemo(() => {
    const value = toTokenInfo.availableAmount?.gt(
  ...
  const maxFromTokenIn = useMemo(() => {
    ...
    return maxFromTokenInUSD
      ?.mul(expandDecimals(1, fromTokenInfo.decimals))
      .div(fromTokenInfo.maxPrice)
      .toString();
  ...
  if (maxToTokenOutUSD && maxFromTokenInUSD) {
    maxSwapAmountUsd = maxToTokenOutUSD.lt(maxFromTokenInUSD)
      ? maxToTokenOutUSD
      : maxFromTokenInUSD;
  }

  let maxShortAmountUsd = undefined;

  if (
    shortCollateralToken &&
    shortCollateralToken.availableAmount &&
    shortCollateralToken.decimals &&
    shortCollateralToken.minPrice
  ) {
    maxShortAmountUsd = shortCollateralToken.availableAmount
      .mul(shortCollateralToken.minPrice)
      .div(expandDecimals(1, shortCollateralToken.decimals));
  }
  if (toTokenInfo.hasMaxAvailableShort) {
    if (
      maxShortAmountUsd &&
      maxShortAmountUsd.lt(toTokenInfo.maxAvailableShort)
    ) {
      // do nothing
    } else {
      maxShortAmountUsd = toTokenInfo.maxAvailableShort;
    }
  }
  ...
  const existingLiquidationPrice = existingPosition
    ...
    : undefined;
  const displayLiquidationPrice = liquidationPrice || existingLiquidationPrice;

  if (hasExistingPosition) {
    const collateralDelta = fromUsdMin || bigNumberify(0);
    const sizeDelta = toUsdMax || bigNumberify(0);
  ...
  const getSwapError = () => {
    ...
    if (!isMarketOrder) {
      if (
        (toToken.isStable || toToken.isUsdph) &&
        (fromToken.isStable || fromToken.isUsdph)
      ) {
        return ["Select different tokens"];
      }
  ...
  const getLeverageError = () => {
    ...
    if (
      leverage &&
      leverage.gt((MAX_LEVERAGE / 10000) * BASIS_POINTS_DIVISOR)
    ) {
      return [`Max leverage: ${MAX_LEVERAGE / 10000}x`];
    }
    ...
    if (isLong) {
      ...
      if (fromTokenAddress !== toTokenAddress) {
        ...
        if (toToken && toTokenAddress !== USDPH_ADDRESS) {
        ...
        if (stableTokenAmount.gt(shortCollateralToken.availableAmount)) {
          return [
            "Insufficient liquidity" +
              (stableTokens.length > 1 ? ', change "Profits In"' : ""),
          ];
        }
  ...
  const renderErrorModal = () => {
    const inputCurrency =
      fromToken.address === AddressZero ? "tPLS" : fromToken.address;
    let outputCurrency;
    if (isLong) {
      outputCurrency =
        toToken.address === AddressZero ? "tPLS" : toToken.address;
    } else {
      outputCurrency = shortCollateralToken.address;
    }
    let externalSwapUrl = "";
    if (chainId === PLS_TESTNET_V2) {
      externalSwapUrl = `https://app.v2b.testnet.pulsex.com/swap?inputCurrency=${inputCurrency}&outputCurrency=${outputCurrency}`;
    }
    const externalSwapName = "PulseX";
    const label =
      modalError === "BUFFER"
    ...
    return (
      <Modal
        ...
        label={label}
        className="Error-modal font-base"
      >
        <div>
          You need to select {swapTokenSymbol} as the "Pay" token to initiate
          this trade.
        </div>
        <br />
        {isShort && stableTokens.length > 1 && (
          <div>
  ...
  const getPrimaryText = () => {
    ...
    if (!isMarketOrder)
      return `Create ${
        orderOption.charAt(0) + orderOption.substring(1).toLowerCase()
      } Order`;
    ...
    if (isLong) {
      ...
      if (indexTokenInfo && indexTokenInfo.minPrice) {
        ...
        if (
          fromTokenAddress === USDPH_ADDRESS &&
          nextToAmountUsd.lt(fromUsdMin.mul(98).div(100))
        ) {
          return "High USDPH Slippage, Long Anyway";
        }
      }
    ...
    if (
      ...
      helperToast.error(
        `Leave at least ${formatAmount(
          ...
          undefined,
          0
        )} ${getConstant(chainId, "nativeTokenSymbol")} for gas`
      );
      return;
    }
  ...
  const createIncreaseOrder = () => {
    ...
    const successMsg = `
      ...
      isLong ? "Long" : "Short"
    }: ${formatAmount(toUsdMax, USD_DECIMALS, 2, undefined, undefined, 0)} USD!
    `;
  ...
  const increasePosition = async () => {
    ...
    const boundedFromAmount = fromAmount || bigNumberify(0);
    ...
    const tokenSymbol = indexToken.isWrapped
      ...
      : indexToken.symbol;
    const successMsg = `Requested increase of ${tokenSymbol} ${
      isLong ? "Long" : "Short"
    } by ${formatAmount(
      toUsdMax,
      USD_DECIMALS,
  ...
  if (isSwap) {
    if (fromAmount) {
      ...
      if (feeBasisPoints !== undefined) {
        ...
        const feeTokenPrice =
          fromTokenInfo.address === USDPH_ADDRESS
  ...
  const hasZeroBorrowFee = false;
  let borrowFeeText;
  if (isLong && toTokenInfo && toTokenInfo.fundingRate) {
    borrowFeeText =
      formatAmount(toTokenInfo.fundingRate, 4, 4, undefined, undefined, 0) +
      "% / 1h";
          ...
          <div className="Exchange-swap-section">
            <div className="Exchange-swap-section-top">
              ...
              <div
                ...
                onClick={() => {
                  setTriggerPriceValue(
                    formatAmountFree(entryMarkPrice, USD_DECIMALS, 4)
                  );
                }}
              >
                Mark: {formatAmount(entryMarkPrice, USD_DECIMALS, 4, true)}
              </div>
            </div>
            <div className="Exchange-swap-section-bottom">
              <div className="Exchange-swap-input-container">
          ...
          <div className="Exchange-swap-box-info">
            <ExchangeInfoRow label="Fees">
              <div>
                ...
                {fees && (
                  <div>
                    {formatAmount(feeBps, 2, 2, false, undefined, 0)}%&nbsp; (
                    {formatAmount(
                      fees,
                      fromToken.decimals,
                      4,
            ...
            <div className="Exchange-info-row">
              <div className="Exchange-info-label">
                <Tooltip
                  ...
                  handle="Liq. Price"
                  renderContent={() =>
                    `Liquidation price is calculated as the price at which the (collateral - losses - borrow fee) is less than ${formatAmount(
                      (10000 * 10000) / MAX_LEVERAGE,
                      2,
                      0,
                      true,
                      undefined,
                      0
                    )}% of your position's size or can not cover liquidation cost.`
                  }
                />
              </div>
              <div className="align-right">
                ...
                renderContent={() => {
                  return (
                    <>
                      ...
                      <br />
                      {/* <a
                        href="https://phamousio.gitbook.io/phamous/trading#opening-a-position"
                        target="_blank"
      ...
      {/* <div className="Exchange-swap-market-box App-box App-box-border">
        ...
        <div className="Exchange-info-row">
          <div className="Exchange-info-label-button">
            <a href="https://phamousio.gitbook.io/phamous/trading" target="_blank" rel="noopener noreferrer">
              Trading guide
            </a>
          </div>
        </div>
        ...
        <div className="Exchange-info-row">
          <div className="Exchange-info-label-button">
            <a href="https://phamousio.gitbook.io/phamous/trading#backup-rpc-urls" target="_blank" rel="noopener noreferrer">
