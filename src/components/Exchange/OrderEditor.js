...
export default function OrderEditor(props) {
  ...
  const position =
    order.type !== SWAP
      ? getPositionForOrder(account, order, positionsMap)
      : null;
  const liquidationPrice =
    order.type === DECREASE && position ? getLiquidationPrice(position) : null;
  ...
  const [triggerRatioValue, setTriggerRatioValue] = useState(
    formatAmountFree(initialRatio, USD_DECIMALS, 6)
  );

  const [triggerPriceValue, setTriggerPriceValue] = useState(
    formatAmountFree(order.triggerPrice, USD_DECIMALS, 4)
  );
  ...
  const getError = () => {
    if (
      (!triggerRatio || triggerRatio.eq(0)) &&
      (!triggerPrice || triggerPrice.eq(0))
    ) {
      return "Enter Price";
    }
    ...
    if (
      ...
      if (
        !order.triggerAboveThreshold &&
        indexTokenMarkPrice.lt(triggerPrice)
      ) {
    ...
    if (order.type === SWAP) {
      ...
      if (currentRate && !currentRate.gte(triggerRatio)) {
        return `Price is ${
          triggerRatioInverted ? "below" : "above"
        } Mark Price`;
      }
    }
  };
  ...
  const renderMinProfitWarning = () => {
    ...
    const { delta, pendingDelta, pendingDeltaPercentage, hasProfit } =
  ...
  return (
    ...
    >
      ...
      <ExchangeInfoRow label="Price">
        {triggerRatio &&
        !triggerRatio.eq(0) &&
        !triggerRatio.eq(order.triggerRatio) ? (
          <>
            <span className="muted">
              {getExchangeRateDisplay(
                order.triggerRatio,
