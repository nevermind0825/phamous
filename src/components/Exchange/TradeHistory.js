...
function renderLiquidationTooltip(liquidationData, label) {
  const minCollateral = liquidationData.size
    ...
    <Tooltip
      ...
      renderContent={() => (
        <>
          ...
          {formatAmount(minCollateral, USD_DECIMALS, 2, true, undefined, 0)}
          <br />
          Borrow fee: $
          {formatAmount(
            liquidationData.borrowFee,
            USD_DECIMALS,
            2,
            true,
            undefined,
...
export default function TradeHistory(props) {
  const { account, infoTokens, getTokenInfo, chainId, nativeTokenAddress } =
    props;
    ...
    (trade) => {
      ...
      if (tradeData.action === "CreateIncreasePosition") {
        ...
        if (bigNumberify(params.sizeDelta).eq(0)) {
          return `Request deposit into ${indexToken.symbol} ${
      ...
      if (tradeData.action === "CreateDecreasePosition") {
        ...
        if (bigNumberify(params.sizeDelta).eq(0)) {
          return `Request withdrawal from ${indexToken.symbol} ${
            params.isLong ? "Long" : "Short"
          }`;
        }
      ...
      if (tradeData.action === "CancelIncreasePosition") {
        ...
        if (bigNumberify(params.sizeDelta).eq(0)) {
          return (
            <>
              Could not execute deposit into {indexToken.symbol}{" "}
              {params.isLong ? "Long" : "Short"}
            </>
          );
        }
        ...
        return (
          <>
            Could not increase {indexToken.symbol}{" "}
            {params.isLong ? "Long" : "Short"},
            {`+${formatAmount(
              params.sizeDelta,
              USD_DECIMALS,
            ...
            <Tooltip
              ...
              renderContent={() => (
                <>
                  Try increasing the "Allowed Slippage", under the Settings menu
                  on the top right
                </>
              )}
            />
      ...
      if (tradeData.action === "CancelDecreasePosition") {
        ...
        return (
          <>
            Could not decrease {indexToken.symbol}{" "}
            {params.isLong ? "Long" : "Short"},
            {`+${formatAmount(
              params.sizeDelta,
              USD_DECIMALS,
      ...
      if (
        ...
        if (isLiquidation && liquidationData) {
          return (
            <>
              ...
              {indexToken.symbol} {params.isLong ? "Long" : "Short"}, -
              {formatAmount(
                params.sizeDelta,
                USD_DECIMALS,
                2,
              ...
              USD, {indexToken.symbol}&nbsp; Price: $
              {formatAmount(params.price, USD_DECIMALS, 4, true)} USD
            </>
          );
        }
        const actionDisplay = isLiquidation
          ? "Partially Liquidated"
          : "Decreased";
        return `
        ${actionDisplay} ${indexToken.symbol} ${
          params.isLong ? "Long" : "Short"
        },
        -${formatAmount(
          params.sizeDelta,
        ...
        ${indexToken.symbol} Price: ${formatAmount(
          params.price,
          USD_DECIMALS,
          4,
          true
      ...
      if (
        ...
        if (liquidationData) {
          return (
            <>
              {renderLiquidationTooltip(liquidationData, "Liquidated")}{" "}
              {indexToken.symbol} {params.isLong ? "Long" : "Short"}, -
              {formatAmount(params.size, USD_DECIMALS, 2, true, undefined, 0)}{" "}
              USD,&nbsp;
              {indexToken.symbol} Price: $
              {formatAmount(params.markPrice, USD_DECIMALS, 4, true)} USD
            </>
          );
        }
        return `
        Liquidated ${indexToken.symbol} ${params.isLong ? "Long" : "Short"},
        -${formatAmount(params.size, USD_DECIMALS, 2, true, undefined, 0)} USD,
        ${indexToken.symbol} Price: ${formatAmount(
          params.markPrice,
          USD_DECIMALS,
          4,
      ...
      if (
        ["ExecuteIncreaseOrder", "ExecuteDecreaseOrder"].includes(
          tradeData.action
        )
      ) {
        ...
        const executionPriceDisplay = formatAmount(
          order.executionPrice,
          USD_DECIMALS,
          4,
          true
        );
        const sizeDeltaDisplay = `${
          order.type === "Increase" ? "+" : "-"
      ...
      if (tradeData.action === "ExecuteSwapOrder") {
        ...
        const fromToken = getTokenInfo(
          infoTokens,
          order.path[0] === nativeTokenAddress ? AddressZero : order.path[0]
        );
        const toToken = getTokenInfo(
          infoTokens,
          order.shouldUnwrap ? AddressZero : order.path[order.path.length - 1]
        );
      ...
      if (
        ["CreateSwapOrder", "UpdateSwapOrder", "CancelSwapOrder"].includes(
          tradeData.action
        )
      ) {
        const order = deserialize(params.order);
        ...
        return `
        ${getOrderActionTitle(tradeData.action)}:
        Swap ${amountInDisplay} ${
          fromToken?.symbol || ""
        } for ${minOutDisplay} ${toToken?.symbol || ""},
        Price: ${getExchangeRateDisplay(
          order.triggerRatio,
          fromToken,
          toToken
        )}`;
  ...
  return (
    <div className="TradeHistory">
      {tradesWithMessages.length === 0 && (
        <div className="TradeHistory-row App-box">No trades yet</div>
      )}
      {tradesWithMessages.length > 0 &&
                ...
                <div className="muted TradeHistory-time">
                  ...
                  {(!account || account.length === 0) && (
                    <span>
                      {" "}
                      (
                      <Link to={`/actions/${tradeData.account}`}>
                        {tradeData.account}
                      </Link>
                      )
                    </span>
